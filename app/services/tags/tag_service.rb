module Tags
  ...
  # @raise [Tags::PermissionDenied] if user lacks create permission
  # @raise [Tags::ValidationFailed] if tag validations fail
  ...
  # @raise [Tags::NotFound] if tag is nil
  # @raise [Tags::PermissionDenied] if user lacks update permission or can't change owner
  # @raise [Tags::ValidationFailed] if tag validations fail or new owner not found
  ...
  # @raise [Tags::NotFound] if tag is nil
  # @raise [Tags::PermissionDenied] if user lacks destroy permission
  # @raise [Tags::ValidationFailed] if destruction fails
  def destroy_tag(tag)
    ensure_exists!(tag)
  ...
  def ensure_exists!(tag, name = nil)
    unless tag
      ...
      raise Tags::NotFound.new(tag_name)
    end
  end
  ...
  def prepare_update_params(tag, tag_params)
    ...
    if discord_uid.present?
      ...
      unless new_owner
        ...
        raise Tags::ValidationFailed.new(tag)
      end
